...
window.HT.__apiContract = Object.freeze({
  ...
  entries: Object.freeze([
    ...
    Object.freeze({
      name: 'HT.scoring.score',
      signature: '(answers: Readonly<{[id: string]: string|number|undefined}>, spec: {traits: ReadonlyArray<string>, weights: Readonly<{[qid: string]: Readonly<{[value: string]: Readonly<{[trait: string]: number}>}>}>, archetypes: ReadonlyArray<{id: string, label?: string, emoji?: string, scores?: Readonly<{[trait: string]: number}>, default?: boolean}>, traitMax?: Readonly<{[trait: string]: number}>}) => Readonly<{traits: Readonly<{[trait: string]: number}>, archetype: Readonly<{id: string, label: string, emoji: string, default: boolean}> | null}>',
      stability: 'stable',
    ...
    Object.freeze({
      name: 'HT.embed',
      signature: 'Readonly<{publish: (opts: Readonly<{instanceUuid: string, slug?: string|null}>) => Readonly<{instanceUuid: string, slug: string|null, postMessage: (envelope: object) => void, on: (type: string, fn: (envelope: object, ev: MessageEvent) => void) => () => void, destroy: () => Readonly<{instanceUuid: string, slug: string|null, postMessage: (envelope: object) => void, on: (type: string, fn: (envelope: object, ev: MessageEvent) => void) => () => void, destroy: () => void}>|null}>|null, openModal: (slug: string, sourceEl?: HTMLElement|null, opts?: Readonly<{previewWidth?: number, previewHeight?: number, previewScale?: number}>) => void, closeModal: () => void, isModalOpen: () => boolean, button: (slug: string) => HTMLButtonElement|null, mount: (slug: string, rootEl: HTMLElement) => Readonly<{teardown: () => void}>}>',
      stability: 'stable',
      module: 'assets/js/embed.js',
      notes: 'Story 4.1 + Story 4.2 — instance-scoped embed API factory PLUS standalone snippet modal. Story 4.1 added publish(): each call returns a NEW frozen object so two embeds on the same host page do not share state (independent UUID + message bus + window.name). The factory requires a non-empty instanceUuid (the UUID generated by the FOUC IIFE when ?embed=<slug> is present) and accepts an optional slug. postMessage() forwards envelopes to window.parent via postMessage with origin "*" — the embed never knows the host origin without prior configuration; the host filters by event.source against its own iframe registry. on(type, fn) registers a window message listener filtered by envelope.type and returns an unsubscribe cleanup function. destroy() disconnects the ResizeObserver owned by shell.js (does NOT clear window.name — shell.js owns the window.name lifecycle so BFCache re-entry re-establishes identity). Story 4.2 added openModal(): opens the standalone snippet modal that renders the <iframe> snippet in <code class="embed-snippet"> (NOT a textarea — Story 3.9 textarea path is kept for backward compatibility) with a Copy button (`<button data-action="copy-snippet">Copy</button>`) that copies the snippet via HT.copyToClipboard and shows the toast "Copied" for 2 seconds. The modal renders a live preview iframe with sandbox="allow-scripts allow-same-origin" (NO allow-top-navigation, NO allow-popups). sourceEl is the Embed button to re-focus on close. opts.previewWidth/previewHeight override the default 320x240 preview sizing (B3 a11y floor). closeModal() closes any open modal. isModalOpen() returns the current open state. button(slug) creates an Embed trigger button. mount(slug, rootEl) inserts the Embed button into .tool-actions next to the Share button (suppressed in ?embed= mode and when the slug has no embed-snippet block). Read-only (Object.freeze + writable:false configurable:false). Loaded unconditionally by every page so HT.embed is reachable from any tool page when a host embeds it with `?embed=<slug>`. Bundle target: ≤ 4 KB gz (raised from 2 KB gz in Story 4.2 to absorb the modal builders). Smoke harness: scripts/_smoke_embed_router.js + scripts/_smoke_embed_modal.js.',
    }),
    Object.freeze({
      name: 'HT.embed.openModal',
      signature: '(slug: string, sourceEl?: HTMLElement|null, opts?: Readonly<{previewWidth?: number, previewHeight?: number, previewScale?: number}>) => void',
      stability: 'stable',
      module: 'assets/js/embed.js',
      notes: 'Story 4.2 — opens the standalone embed snippet modal for the given slug. The modal renders the iframe snippet per spec (URL has `?embed=<slug>`, style="border:0", aria-label, loading="lazy") in a <code class="embed-snippet"> element with a Copy button (`<button data-action="copy-snippet">Copy</button>`) that copies via HT.copyToClipboard and toasts "Copied" for 2 seconds. Live preview iframe below uses sandbox="allow-scripts allow-same-origin" (NO allow-top-navigation, NO allow-popups). sourceEl is re-focused on close. Suppressed in `?embed=` mode (the embed visitor cannot embed-it-further). Read-only — no public API additions beyond what HT.embed.publish already exposed. Bundle target: ≤ 4 KB gz for the whole embed.js module. Smoke harness: scripts/_smoke_embed_modal.js.',
    }),
    Object.freeze({
      name: 'HT_EMBED_VERSION',
      signature: 'Readonly<{version: "1.1.0"}>',
      stability: 'internal',
      module: 'assets/js/embed.js',
      notes: 'Story 4.1 + Story 4.2 internal handle (AD-14 internal-handle pattern; mirrors HT_HISTORY_INIT + HT_EXPORT_SCHEMA_VERSION + HT_IMPORT_DIALOG_VERSION). Exposes the embed module bootstrap tuple {version} so the smoke harness can verify the loaded module matches the api-contract version. Frozen via Object.freeze at module load. The `version` field is the literal "1.1.0" (Story 4.1 was "1.0.0"; Story 4.2 bumped for the openModal surface addition). It is the embed envelope shape contract version. Future migrations (e.g., adding origin allowlist, payload validation) bump this number so the smoke harness + host pages can detect a contract drift. NOT a public HT.* surface — accessing this from a tool is undefined behavior.',
    }),
  ]),
});
