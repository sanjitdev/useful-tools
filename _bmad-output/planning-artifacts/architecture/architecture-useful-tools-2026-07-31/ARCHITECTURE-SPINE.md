...
- **Extension (Story 2.3 + 3.6):** Per-tool history persistence + replay is also a Shell-owned global concern. The Shell exposes `HT.history.push` / `list` / `restore` / `clear` / `subscribe` / `panel` / `button` / `hasHistory` as the single source of truth for "what the user ran before and how to replay it" on any tool page. A Tool pushes its current state via `HT.history.push(slug, { inputs, result })`, restores via `HT.history.restore(slug, entry-or-ts)`, and gets its "Show history" affordance via `HT.history.button(slug)`. The Shell owns the FIFO cap of 50 (Story 3.6 raised from Story 2.3's 10 per FR-12 + retention finding), the `handy-tools.history.<slug>` key family, the per-tool storage isolation, the cross-tab `storage` event sync, the desktop sidebar / mobile sheet split, the panel DOM rendering (Story 3.6 added a close button + backdrop to the desktop sidebar; rows now carry a `<time>` element with relative timestamps via `Intl.RelativeTimeFormat(navigator.language, {numeric: 'auto'})` falling back to `Intl.DateTimeFormat` after 7 days), the read-time migration of legacy `{id, ts (number), state, result, label}` entries to the new `{ts (ISO 8601), inputs, result}` shape (routes through `HT.storage.set` so the bypass-check ban is honored), and the restore-confirm `<dialog>` UX with Cancel-first button order. A Tool never reads/writes `handy-tools.history.<slug>` directly via `localStorage`, never rolls its own ring buffer, never manages a history list UI, and never binds its own `(h)` shortcut. The history-panel gate (`scripts/shell-bounds-check.py` HISTORY_KEY_* patterns) consumes the Shell-owned key family to verify that no `tools/<slug>/<slug>.js` reaches for `handy-tools.history.*` keys directly.
- **Extension (Story 2.5):** Per-tool Share + Print + Embed-Code affordances are also Shell-owned global concerns. The Shell exposes `HT.share.open` / `close` / `isOpen` / `url` / `embedCode` / `button` / `hasShare` / `mount` / `print` (the canonical Print wrapper for legacy tools with custom Print buttons) as the single source of truth for "how a user shares this tool's result" on any tool page. The Shell owns the `<dialog>` markup (the same pattern Story 2.2's destructive-confirm and Story 2.3's history-clear used), the `embed-snippet` block reader, the toast on Copy URL, the `window.print()` invocation (sanctioned via `HT.share.print`), the `loading="lazy"` iframe for embed-code, the print-stylesheet hide-chrome block (Story 2.5 appends a `@media print` block to `assets/css/base.css`), the focus-trap on `showModal()`, and the `embed=1` skip. A Tool never binds its own `#share-dialog`, never injects "Copy URL" / "Print" / "Embed Code" / "Share tool" strings into the DOM, never reaches for `window.print()` or `navigator.clipboard.writeText()` directly (use `HT.share.print(slug)` or `HT.copyToClipboard(text)` from `utils.js`), and never calls `HT.share.open/.close/.embedCode/.button/.mount` (the Shell mounts at boot via `HT.share.mount(slug, main)`). The share-dialog gate (`scripts/shell-bounds-check.py` SHARE_DIALOG_* / HT_SHARE_DIRECT_RE / WINDOW_PRINT_RE / NAV_CLIPBOARD_WRITE_RE patterns) consumes the Shell-owned surface to verify that no `tools/<slug>/<slug>.js` builds ad-hoc share UI.
- **Extension (Story 3.7 + 3.8):** User data Export (Story 3.7) and Import (Story 3.8) are also Shell-owned global concerns. The Shell exposes `HT.export.run()` (Story 3.7) and the corresponding `HT.import.run()` (Story 3.8) as the single source of truth for "how a user moves their data between devices" on any tool page. The Shell owns the JSON payload shape (`{version, exportedAt, settings, history, favorites, recent, pins}`), the `HT_EXPORT_SCHEMA_VERSION = "1.0.0"` internal handle (mirrors `HT_HISTORY_INIT`), the hand-rolled 6-check validation predicate (no ajv vendoring — AD-1, the predicate is small enough to be hand-rolled), the Blob + `<a download>` + `URL.revokeObjectURL` 1-second revocation (the delay races the browser's download commit on slow clients), the `handy-tools-export-YYYY-MM-DD.json` filename in the user's local timezone (UTC silently shifts the date for half the world; do not use), the `?embed=1` / `HT_SHELL_EMBED` early-return guard (the Settings modal is already blocked from opening in embed mode per AD-7 — the export is a defense-in-depth no-op), the read-only contract (the export never writes to `localStorage`), and the reversible-action UX (no typed confirmation — UX-DR-3; the success toast + the file landing in the browser's Downloads folder are the only feedback). A Tool never reads `HT.storage` to assemble its own export payload, never calls `URL.createObjectURL` to trigger a download, never hand-rolls a `<a download>` click handler, and never sets `data-ht-action="export-data"` on its own buttons (the Settings modal's Export button is the single insertion point). The export-module gate (`scripts/_smoke_export.js`) verifies the frozen `HT.export` surface, the `HT_EXPORT_SCHEMA_VERSION` bootstrap handle, the payload assembly, the validation-failure path (toast + `console.error` + no download), and the embed-mode guard.
...
- **Extension (Story 2.5):** The canonical Share URL is `location.href`. `HT.share.url(slug)` is a one-liner that returns `window.location.href` — the URL is already canonical because `HT.urlState.bindForm` + `HT.urlState.subscribe` keep the fragment hash synchronized with the form state. There is no construction: the Shell does not assemble the URL, the browser does. `HT.share.embedCode(slug)` reads `tools.json`'s `embed-snippet` block (`{min-width, min-height}` — minimum 240 per B3 a11y, `title`) and emits the iframe snippet — the same codec grammar (fragment hash) applies on load because the embed's `src` URL is `tools/<slug>/` and the embedded iframe's `bindForm` picks up the codec hash automatically.
...
- **Rule:** Every `localStorage` key is registered in a single `assets/js/storage-registry.js` as `HT.storage.register('namespace.key', { purpose, lifetime, schema, owner })`. The registry rejects duplicate registrations at boot. A Tool/Shell calls only `HT.storage.get/set/del` with a registered key. **Ownership**: each registered key has exactly one `owner` module. Other modules read/write through the owner's public API (e.g., `HT.history.push` for the per-tool history key), never through `HT.storage.set` directly on a non-owned key. Two namespaces: `ht.*` for runtime state (theme, locale, settings), `handy-tools.*` for user data (history, favorites, recent, pins). The Shell owns the shared keys (`handy-tools.history.<slug>`, `handy-tools.favorites`, `handy-tools.recent`, `handy-tools.pins`, `handy-tools.dashboard`, `handy-tools.settings`); Tools own only their own history keys and read shared keys through `HT.*` APIs. The existing `ht.theme` key moves into the registry as `ht.theme` owned by `theme.js`. `[ASSUMPTION: existing 'ht.theme' is grandfathered into the registry without a migration step; we treat its absence as 'auto'.]`
- **Clarification (Story 2.3 + 3.6):** Although every `handy-tools.history.<slug>` key is conceptually "the Tool's", the **owner module is `assets/js/history.js`** (the Shell), not the Tool. The Tool never registers a per-slug key in `storage-registry.js`; `history.js` registers one entry per slug that has both `history-keys` AND a `urlState` block (the `hasHistory()` AND-gate) at boot, on behalf of all Tools. Tools interact with their history only through `HT.history.push` / `list` / `restore` / `clear` / `subscribe` / `panel` / `button` / `hasHistory` (AD-14 frozen surface). This keeps the per-tool isolation + FIFO cap (50, Story 3.6) + cross-tab sync logic in one place. Story 3.6's read-time migration of legacy entries also routes through `HT.storage.set` (not `localStorage.setItem`) so the bypass-check ban on `handy-tools.history.*` keys is honored even during migration. The bypass check (`scripts/shell-bounds-check.py` HISTORY_KEY_* patterns) enforces the no-direct-`localStorage` rule on `handy-tools.history.*` keys.
