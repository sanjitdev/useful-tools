...
- **Extension (Story 2.3):** Per-tool history persistence + replay is also a Shell-owned global concern. The Shell exposes `HT.history.push` / `list` / `restore` / `clear` / `subscribe` / `panel` / `button` / `hasHistory` as the single source of truth for "what the user ran before and how to replay it" on any tool page. A Tool pushes its current state via `HT.history.push(slug, { state, result, label })`, restores via `HT.history.restore(slug, id)`, and gets its "Show history" affordance via `HT.history.button(slug)`. The Shell owns the FIFO cap of 10 per FR-12, the `handy-tools.history.<slug>` key family, the per-tool storage isolation, the cross-tab `storage` event sync, the desktop sidebar / mobile sheet split, and the panel DOM rendering. A Tool never reads/writes `handy-tools.history.<slug>` directly via `localStorage`, never rolls its own ring buffer, never manages a history list UI, and never binds its own `(h)` shortcut. The history-panel gate (`scripts/shell-bounds-check.py` HISTORY_KEY_* patterns) consumes the Shell-owned key family to verify that no `tools/<slug>/<slug>.js` reaches for `handy-tools.history.*` keys directly.
