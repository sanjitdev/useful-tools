...
help: ## Show available targets
	...
	@echo "  make paint-calculator-smoke Run the Node vm-context smoke for the Paint Calculator tool (Story 9.11)"
	...
	@echo "  make check-pack-composition Run the pack-composition gate (Story 9.16 / 9.17) — asserts travel=5 + finance/study/developer/household ≥ 5 + taxonomy"
	@echo "  make ci                  Run validate + rubric-all + gate + site-config + site-config-smoke + storage-registry + shell-drift + shell-a11y + verify-compound + compound-smoke + shell-bounds + shell-bounds-self-test + shell-public-api-smoke + sample-data-smoke + a11y-smoke + a11y-audit + history-smoke + share-dialog-smoke + wave-1-smoke + wave-2-smoke + wave-3-smoke + pack-tags-smoke + check-pack-taxonomy + check-pack-composition + quality-smoke + regression-sweep + regression-sweep-negative + palette-search-smoke + palette-search-smoke-html + palette-actions-smoke + help-overlay-smoke + global-chords-smoke + print-smoke + view-source-smoke + pins-recent-smoke + search-perf-smoke + ast-gates-self-test + ast-gates-negative + script-load-order + paint-calculator-smoke + budget-planner-smoke + savings-goal-smoke + currency-converter-smoke"
...
# `bundle-size-per-tool` is Story 4b Phase 4's per-tool budget gate.
# Measures every tool's <slug>-core.js + vendor deps + per-tool CSS.
# Fails if any tool's core exceeds 7,000 bytes gz (the Story 4b per-tool
# budget) or first-paint payload exceeds 30,000 bytes gz (PRD NFR-1).
# Walks tools/<slug>/index.html, resolves ./<slug>-core.js (or the
# graceful ./<slug>.js fallback for non-splittable tools), detects
# handlers.js drift if it's eagerly loaded in HTML.
# See _bmad-output/implementation-artifacts/story-4b-per-tool-code-splitting.md
# for the budget rationale + tier breakdown.
bundle-size-per-tool:
	@$(PYTHON) scripts/_bundle_size_per_tool.py

# `bundle-size-tools` is a top-N diagnostic; pairs with bundle-size-per-tool.
# Lists all tools sorted by core gz size with first-paint totals.
bundle-size-tools:
...
# Verifies animal-race-core.js exposes HT.animalRaceCore (data + boot)
# and animal-race-handlers.js binds window.animalRaceInit.
# 99 PASS expected.
animal-race-split-smoke:
	@node scripts/_smoke_animal_race_split.js

# Story 4b Phase 3 — recipe-scaler core+handlers split smoke.
# Verifies recipe-scaler-core.js exposes HT.recipeScalerCore (data + math)
# and recipe-scaler-handlers.js binds window.recipeScalerInit.
# 57 PASS expected.
...
# Verifies timestamp-converter-core.js exposes HT.timestampConverterCore
# (classify/formatOutputs/FORMAT_LABELS) and ...-handlers.js binds
# window.timestampConverterInit. 41 PASS expected.
timestamp-converter-split-smoke:
	@node scripts/_smoke_timestamp_converter_split.js

# Story 4b Phase 4 — uuid-generator core+handlers split smoke.
# Verifies uuid-generator-core.js exposes HT.uuidGeneratorCore
# (uuidV1/v4/v7/ulid, validate, clampCount, GENERATORS dispatch)
# and uuid-generator-handlers.js binds window.uuidGeneratorInit.
# 47 PASS expected.
uuid-generator-split-smoke:
	@node scripts/_smoke_uuid_generator_split.js

# Story 4b Phase 4 — json-formatter core+handlers split smoke.
# Verifies json-formatter-core.js exposes HT.jsonFormatterCore
# (sortKeysRecursive, parseSafe, readFeatures, escapeHtml) and
# json-formatter-handlers.js binds window.jsonFormatterInit.
# 41 PASS expected.
json-formatter-split-smoke:
	@node scripts/_smoke_json_formatter_split.js

# Story 4b Phase 4 — grocery-list core+handlers split smoke.
# Verifies grocery-list-core.js exposes HT.groceryListCore
# (CATEGORIES, SAMPLE_ITEMS, encodeList, decodeList, escapeHtml,
# readUrlState, isReducedMotion) and ...-handlers.js binds
# window.groceryListInit. 48 PASS expected.
grocery-list-split-smoke:
	@node scripts/_smoke_grocery_list_split.js
...
# Story 9.11 — Paint Calculator (walls, doors, windows, Math.ceil coverage
# round-up, base64 JSON walls in URL state, add-wall + remove-wall buttons,
# debounced 120ms input, reduced-motion, keyboard shortcuts s/r/p/c).
# Vacuous-pass guard catches hollow runs.
paint-calculator-smoke:
	@node scripts/_smoke_paint_calculator.js

# Story 9.12 — Area & Volume Calculator (rectangle, triangle, circle,
# L-shape, box-3d, cylinder-3d; imperial ↔ metric conversion; shape radio
# switching; URL state encode/decode/resolve; reduced-motion CSS; no fetch
# / XHR / localStorage).
area-volume-smoke:
	@node scripts/_smoke_area_volume.js

# Story 9.13 — Budget Planner (income + 5 default categories; compute
# totalExpenses / savings / savingsRate / discretionary; addCategory UUID
# stability; removeCategory / updateCategory; base64 URL state round-trip;
# reduced-motion CSS; privacy — no fetch / XHR / console.error).
budget-planner-smoke:
	@node scripts/_smoke_budget_planner.js

# Story 9.14 — Savings Goal (annuity-due math with / without compound
# interest; URL state encode / decode / resolve; validation rejects
# target=0 / months=0 / NaN; reduced-motion CSS; privacy — no fetch /
...
# Story 9.16 / 9.17 — pack composition gate. Asserts the Travel pack
# contains EXACTLY the 5 AC-mandated tools (currency-converter,
# tip-calculator, unit-converter, recipe-scaler, exam-countdown) and
# Finance / Study / Developer / Household packs each have ≥ 5 ready
# tools. Pure-stdlib Python, exits 0 (pass), 2 (missing required),
# 3 (vacuous), 4 (taxonomy), 5 (below-minimum).
...
# Story 4c — quiz.js + quiz.css lazy-load smoke. Verifies the
# shell-thin.js Proxy factory wiring for quiz: HT.quiz is a Proxy,
# first access fires lazyLoad('assets/js/quiz.js') + lazyLoadCss
# ('assets/css/quiz.css') in parallel, and the bundle-size gate
# has moved quiz into SPEC_PAGE_CONDITIONAL_MODULES with the
