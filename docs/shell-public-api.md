...
| `HT.challenge` | stable | challenge.js (DC-3 / Story 10.4 — Challenge-a-Friend viral-loop protocol: `link(spec) → string` returns a repo-relative URL with `?c=<base64url-blob>` encoding `{v: 1, slug, self, iat, exp}`; default `exp` is `iat + 30 days`. The blob only encodes `self` (no about-side answers until the friend submits); no free-text, no PII. `compare(selfA, selfB) → {score: 0..100, axes: [{qid, a, b, delta}]}` where score is Jaccard-style agreement %; deterministic across calls. `verify(blob)` returns one of `{ok: true}`, `{ok: false, code: "malformed"}`, `{ok: false, code: "spec-mismatch", version, supported, message: "newer or older version of the quiz"}`, `{ok: false, code: "expired", exp, message: "this challenge has expired"}`. Page-conditional — loaded by the shell-thin Proxy factory on first `HT.challenge.link()` call. Bundle target: ≤ 7 KB gz. Smoke harness: `scripts/_smoke_challenge.js`) |
| `HT.recommend` | stable | recommend.js (DC-4 / Story 10.5 — recommendation engine: `match(profile, domain) → {top, alternatives, explain}`. profile is `{traits: {trait: weight 0..1}}` (optional secondary `weights` field reserved for Story 10.10); domain is `"car"` or `"bike"`. Scoring is dot-product: `profile.traits` dot `entryTraitVector` (built from `assets/data/catalog-profiles.json` `domains[domain].attrMap`), normalized to `[0, 100]`. Tie-break by id alphabetic for determinism. Returns `top: null` when the domain catalog is empty; `alternatives` is length ≥ 1 when the catalog has ≥ 2 entries. `explain.whyMatch` is the top-2 trait contributions; `explain.whyNot` is the bottom-1 trait. Page-conditional — loaded by the shell-thin Proxy factory on first `HT.recommend.match()` call alongside `HT.catalog`. Bundle target: ≤ 4 KB gz. Smoke harness: `scripts/_smoke_recommend.js`) |
| `HT.catalog` | stable | catalog.js (DC-4 / Story 10.5 — domain catalog lookup. `list() → {car: N, bike: N}` returns the canonical count map (≥ 10 each per the DC-4 contract); `lazyLoad(domain) → [{id, domain, attrs{}, why}]` returns the frozen entry list for a single domain. The data is bundled as JSON in `assets/data/` (cars.json, bikes.json, catalog-profiles.json) — the catalog is intentionally local (no fetch, no http URLs) so the offline `file://` path works the same as the GitHub Pages path. `_entries` + `_profiles` are internal handles used by `HT.recommend.match`; tools should call `HT.recommend.match`, not `HT.catalog` directly. Page-conditional — loaded by the shell-thin Proxy factory alongside `HT.recommend` on first `match()` call. Bundle target: ≤ 4 KB gz. Smoke harness: `scripts/_smoke_recommend.js`) |
