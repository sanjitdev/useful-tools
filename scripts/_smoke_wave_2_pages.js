/* ============================================
   Smoke harness for Story 2.7 — Wave-2 pages.

   For each of the 15 wave-2 tools (sorted by JS
   bytes desc, matching the Wave-2 selection),
   verifies:
     ...
     4. tools/<slug>/<slug>.js exists, is non-empty,
        ...
        the Wave-2 differentiator vs. Wave-1).
...
// 15 wave-2 tools, sorted by JS bytes desc — must mirror
// scripts/_promote_wave_2.py + _print_css_bootstrap.py.
const WAVE_2 = [
  'bd-tax-calculator',
  'animal-race',
  'space-calculator',
...
console.log('wave-2-smoke: verifying 15 wave-2 tools (sorted by JS bytes desc)');

const toolsDoc = loadToolsJson();
check('tools.json parses', toolsDoc !== null);
if (toolsDoc === null) {
  console.log('');
  console.log('wave-2-smoke: cannot read tools.json — exit 1');
  process.exit(1);
}

WAVE_2.forEach(function (slug) {
  console.log('');
  console.log('--- ' + slug + ' ---');
...
console.log('');
if (pass === 0 && fail === 0) {
  console.log('wave-2-smoke: vacuous-pass guard tripped (0 PASS, 0 FAIL) — exit 1');
  process.exit(1);
}

console.log('wave-2-smoke: ' + pass + ' PASS, ' + fail + ' FAIL');
