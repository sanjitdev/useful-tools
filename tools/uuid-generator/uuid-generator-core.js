/* ============================================
   UUID Generator — uuid-generator-core.js (Story 4b Phase 4)
   Parse-time core: constants, regexes, pure-math generators
   (uuidV1/v4/v7, ulid, encodeBase32, formatUuid), validators,
   GENERATORS dispatch, clampCount. Handlers read these via
   HT.uuidGeneratorCore.
...
(function () {
  ...
  // -------------------------------------------------------------
  // Expose data tables + pure functions to handlers (AD-14 internal handle).
  // -------------------------------------------------------------
  HT.uuidGeneratorCore = Object.freeze({
    getUUIDV147Regex: function () { return UUID_V147_RE; },
    getUlidRegex: function () { return ULID_RE; },
    getCrockfordAlphabet: function () { return CROCKFORD_ALPHABET; },
    getGenerators: function () { return GENERATORS; },
    uuidV1: uuidV1,
    uuidV4: uuidV4,
    uuidV7: uuidV7,
    ulid: ulid,
  ...
  function boot() {
    ...
    HT.lazyLoadTool('uuid-generator', './uuid-generator-handlers.js').then(function () {
      if (typeof window.uuidGeneratorInit === 'function') {
        try { window.uuidGeneratorInit(); }
        catch (err) {
          if (typeof console !== 'undefined' && console.warn) {
            console.warn('uuid-generator-core: uuidGeneratorInit threw', err);
          }
        }
      }
    }).catch(function (err) {
      if (typeof console !== 'undefined' && console.warn) {
        console.warn('uuid-generator-core: lazyLoadTool failed', err);
      }
    });
  }
